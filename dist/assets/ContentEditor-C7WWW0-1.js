import { bl as commonjsGlobal, bm as getDefaultExportFromCjs, r as reactExports, j as jsxRuntimeExports } from './index-BG4-akrH.js';
...
function requireLodash_clonedeep () {
	if (hasRequiredLodash_clonedeep) return lodash_clonedeep$1.exports;
	...
	(function (module, exports$1) {
		...
		/** Detect free variable `exports`. */
		var freeExports = 'object' == 'object' && exports$1 && !exports$1.nodeType && exports$1;
	...
	return lodash_clonedeep$1.exports;
}
...
function requireLodash_isequal () {
	if (hasRequiredLodash_isequal) return lodash_isequal$1.exports;
	...
	return lodash_isequal$1.exports;
}
...
const ContentEditor = ({ value, onChange }) => {
  ...
  reactExports.useEffect(() => {
    if (!quillRef.current && containerRef.current) {
      ...
      toolbar?.addHandler("image", () => {
        ...
        input.onchange = () => {
          ...
          const reader = new FileReader();
          reader.onload = () => {
            const editor2 = quillRef.current;
            if (!editor2) return;
            const range = editor2.getSelection(true);
            const dataUrl = reader.result;
            const insertIndex = range ? range.index : editor2.getLength();
            editor2.insertEmbed(insertIndex, "image", dataUrl, "user");
            editor2.setSelection(insertIndex + 1, 0, "silent");
          };
          reader.readAsDataURL(file);
        };
        input.click();
    ...
    const handleChange = () => {
      const html = editor?.getSemanticHTML?.() || editor?.root?.innerHTML || "";
      onChange(html);
    };
  ...
  reactExports.useEffect(() => {
    ...
    const currentHtml = quillRef.current?.getSemanticHTML?.() || quillRef.current?.root?.innerHTML || "";
    if (value !== currentHtml) {
      const delta = quillRef.current.clipboard.convert({ html: value || "" });
      quillRef.current.setContents(delta, "silent");
    }
  }, [value]);
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bg-white border border-gray-200 rounded-lg", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
    "div",
    {
      ref: containerRef,
